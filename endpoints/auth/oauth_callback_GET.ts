...
const SUPER_ADMIN_EMAILS = ["jabosaag@gmail.com"];
  ...
  try {
    ...
    if (existingUsers.length > 0) {
      ...
      if (user.status === "suspended") {
        console.log("Suspended user trying to OAuth login - blocking");
      ...
      if (user.status === "deactivated") {
        console.log("Deactivated user trying to OAuth login - blocking");
      ...
      const isSuperAdmin = SUPER_ADMIN_EMAILS.includes(mappedUserData.email);
      await db
        .updateTable("users")
        .set({
